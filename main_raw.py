...
def main_func(dataset_name='Chamelon',eps=[0.5,1,1.5,2,2.5,3,3.5],e1_r=1/3,e2_r=1/3,N=20,t=1.0,exp_num=10,save_csv=False):
    ...
    for ei in range(len(eps)):
        ...
        for exper in range(exp_num):
            ...
            print('Nodes=%d,Edges=%d/%d (diff=%+d),nmi=%.4f,cc_rel=%.4f,deg_kl=%.4f,mod_rel=%.4f,evc_overlap=%.4f,evc_MAE=%.4f,diam_rel=%.4f' \
                %(mat2_node,mat2_edge,mat0_edge,mat2_edge-mat0_edge,nmi,cc_rel,deg_kl,mod_rel,evc_overlap,evc_MAE,diam_rel))
...
if __name__ == '__main__':
    ...
    # main_func(dataset_name=dataset_name,eps=eps,e1_r=0.2,e2_r=0.6,N=n1,t=t,exp_num=exp_num)
